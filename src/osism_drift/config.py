...
def to_remote_only(config: Config) -> Config:
    """Config for --remote-only: clear base_dirs so every read goes remote.

    Preserves all other fields (sources/releases/release_refs/remote_fallback)
    via dataclasses.replace.
    """
    return replace(config, base_dirs=())
