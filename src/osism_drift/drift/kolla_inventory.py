...
ansible/inventory/multinode (pinned stable/2025.2) against the union of group
names in the OSISM inventory files generics/inventory/50-kolla and 51-kolla. A
group present upstream but absent locally is flagged, and its upstream members
ride in `expected` so a maintainer knows what to add.
...
DESCRIPTION = (
    ...
    "(stable/2025.2) that are absent from the OSISM 50/51-kolla inventory."
)
INPUT_FILES = [
...
_EXPECTED_SRC = "openstack/kolla-ansible/ansible/inventory/multinode @ stable/2025.2"
    ...
    for group in sorted(set(upstream) - local):
        ...
        d = DriftEntry(
            ...
            expected=(
                ...
                f"stable/2025.2; members: {member_str}"
            ),
