...
def test_to_remote_only_clears_base_dirs_preserves_rest(tmp_path):
    import dataclasses
    from osism_drift.config import to_remote_only
...
""",
    ...
    cfg = dataclasses.replace(cfg, base_dirs=("/some/root",), remote_fallback=True)
    ro = to_remote_only(cfg)
    assert ro.base_dirs == ()
    assert ro.remote_fallback is True  # only base_dirs is cleared
    assert ro.releases == ("2025.2",)
    assert ro.release_refs == {"kolla": {"2024.2": "2024.2-eol"}}
    assert ro.sources == cfg.sources and ro.plugins == cfg.plugins
